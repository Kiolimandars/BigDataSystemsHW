...
public class DependencyMiner extends AbstractBehavior<DependencyMiner.Message> {
	...
	private Behavior<Message> handle(BatchMessage message) {
		...
		if (message.getBatch().size() != 0) {
			...
			filesRead += 1;
			this.getContext().getLog().info(filesRead +  (filesRead == 1?" File Read": "Files read"));

			if (filesRead >= inputFiles.length) {
				this.getContext().getLog().info("Read all files");
	...
	private Behavior<Message> handle(CompletionMessage message) {
		...
		this.getContext().getLog().info("Received result " + this.resultsRecieved + "/" + colCombinations);
	...
	private void end() {
		...
		this.getContext().getLog().info("Finished mining within {} s!", discoveryTime/1000);
	}
